...
class ChatService:
    ...
    async def send_conversation(self):
        try:
            ...
            r = await self.s.post_stream(url, headers=self.chat_headers, json=self.chat_request, timeout=chat_conversation_timeout, stream=True)
            if r.status_code != 200:
                rtext = await r.atext()
                if "application/json" == r.headers.get("Content-Type", ""):
